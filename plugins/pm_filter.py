    ...
    if 2 < len(message.text) < 100:    
        ...
        if len(btn) > 10: 
            ...
            if poster:
                ...
                await message.reply_text(f"<b>👇നിങ്ങൾ ചോദിച്ച മൂവി ലഭിക്കാൻ താഴെ കാണുന്ന ബട്ടണിൽ ക്ലിക്ക് ചെയ്യുക😇 {search} ‌‌‌‌‎ ­  ­  ­  ­  ­  </b>", reply_markup=InlineKeyboardMarkup(buttons))
            return
        ...
        if API_KEY:
            poster=await get_poster(search)
        if poster:
            await message.reply_text(f"<b>Here is What I Database For Your Query {search} ‌‎ ­  ­  ­  ­  ­  </b>", reply_markup=InlineKeyboardMarkup(buttons))
